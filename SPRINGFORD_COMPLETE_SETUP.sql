-- ============================================
-- COMPLETE SPRINGFORD DATABASE SETUP
-- ============================================
-- Run this in the SPRINGFORD/NEWS SITE database (NOT diffuse!)
-- This sets up: Admin levels, User deletion, Analytics tracking
-- ============================================

-- STEP 1: Verify you're in the right database
-- You should see tables like: articles, user_profiles, ads
SELECT 'Checking database...' AS status;
SELECT table_name FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('articles', 'user_profiles', 'ads')
ORDER BY table_name;

-- If you see those 3 tables, you're in the right place!
-- If not, STOP and switch to the Springford database

-- ============================================
-- PART 1: SETUP ADMIN LEVELS & USER DELETION
-- ============================================

-- Add admin levels to the enum
ALTER TYPE user_level ADD VALUE IF NOT EXISTS 'admin';
ALTER TYPE user_level ADD VALUE IF NOT EXISTS 'super_admin';

-- Set super admins (you, John, Preston)
UPDATE user_profiles SET user_level = 'super_admin' WHERE email = 'dylancobb2525@gmail.com';
UPDATE user_profiles SET user_level = 'super_admin' WHERE email = 'johnmcguire04@gmail.com';
UPDATE user_profiles SET user_level = 'super_admin' WHERE email ILIKE '%prestonschlagheck@gmail.com%';
...
CREATE POLICY "Super admins can delete users"
  ...
  USING (
    EXISTS (
      ...
      WHERE id = auth.uid() AND user_level = 'super_admin'
    )
  );

SELECT 'Admin levels setup complete!' AS status;

-- ============================================
-- PART 2: SETUP ANALYTICS TRACKING
-- ============================================
...
-- Create article_scroll_data table
...
-- Create policies to allow inserts (CRITICAL!)
DROP POLICY IF EXISTS "Allow all inserts to page_views" ON page_views;
CREATE POLICY "Allow all inserts to page_views" ON page_views FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Allow all inserts to ad_impressions" ON ad_impressions;
CREATE POLICY "Allow all inserts to ad_impressions" ON ad_impressions FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Allow all inserts to ad_clicks" ON ad_clicks;
CREATE POLICY "Allow all inserts to ad_clicks" ON ad_clicks FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Allow all inserts to author_clicks" ON author_clicks;
CREATE POLICY "Allow all inserts to author_clicks" ON author_clicks FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Allow all inserts to section_clicks" ON section_clicks;
CREATE POLICY "Allow all inserts to section_clicks" ON section_clicks FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Allow all inserts to article_scroll_data" ON article_scroll_data;
CREATE POLICY "Allow all inserts to article_scroll_data" ON article_scroll_data FOR INSERT WITH CHECK (true);
...
DROP POLICY IF EXISTS "Service role full access on ad_impressions" ON ad_impressions;
CREATE POLICY "Service role full access on ad_impressions" ON ad_impressions FOR ALL USING (true);

DROP POLICY IF EXISTS "Service role full access on ad_clicks" ON ad_clicks;
...
SELECT 'Analytics tables setup complete!' AS status;
...
-- Show super admins
SELECT 'Super Admins:' AS label, full_name, email FROM user_profiles WHERE user_level = 'super_admin';

-- Show analytics tables created
SELECT 'Analytics Tables Created:' AS label;
SELECT table_name FROM information_schema.tables 
WHERE table_schema = 'public' 
