...
/** Table default: newest activity first (sent → updated → created). */
function campaignRecencyMs(c: Campaign): number {
  const iso = c.sent_at || c.updated_at || c.created_at;
  return new Date(iso).getTime();
}

function templateRecencyMs(t: Template): number {
...
function NewsletterInner() {
  ...
  const sortedFilteredCampaigns = useMemo(() => {
    ...
    if (campSortCol && campSortPhase !== 0) {
      ...
      rows.sort((a, b) => campaignRecencyMs(b) - campaignRecencyMs(a));
    }
    return rows;
  ...
  const sortedFilteredTemplates = useMemo(() => {
    ...
    if (tmplSortCol && tmplSortPhase !== 0) {
      ...
      rows.sort((a, b) => templateRecencyMs(b) - templateRecencyMs(a));
    }
    return rows;
