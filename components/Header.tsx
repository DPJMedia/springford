...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-20 bg-white">
      ...
      <div className="border-b border-[color:var(--color-border)] bg-white backdrop-blur">
        <div className="mx-auto grid w-full max-w-none grid-cols-[1fr_auto_1fr] items-center gap-2 px-3 py-2 sm:px-6 lg:px-8">
          ...
          <div className="flex min-w-0 items-center justify-end gap-2">
            {/* Hamburger layout: anything smaller than desktop (below 1280px) */}
            <div className="flex xl:hidden items-center gap-2">
              {user ? (
                <div className="relative user-menu">
                  <button
                    onClick={() => setShowMenu(!showMenu)}
                    className="flex items-center gap-1.5 rounded-full bg-gray-100 px-2 py-1.5 hover:bg-gray-200 transition"
                  >
                ...
                <Link href="/login" className="rounded-full bg-[color:var(--color-riviera-blue)] px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-opacity-90">
                  Log in
                </Link>
              )}
              <button
      ...
      {/* Hamburger dropdown - sections, Advertise, Support, Subscribe (shown below 1280px) */}
      {showMobileNav && (
