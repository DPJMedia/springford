...
export function createSupportCancelToken(
  ...
  const payload = Buffer.from(
    JSON.stringify({ sub: subscriptionId, uid: userId, em: email, exp }),
    "utf8"
  ).toString("base64url");
...
export function verifySupportCancelToken(
  token: string
): { sub: string; uid: string; em: string } | null {
  const secret = getSecret();
  ...
  return { sub: data.sub, uid: data.uid, em: data.em };
}
